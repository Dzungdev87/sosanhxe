...
const BRAND_URL = "https://vnexpress.net/oto-xe-may/v-car/hang-xe/vinfast-33";
...
async function main() {
  ...
  const cars = await fetchVinFastListings();
  console.log(`Found VinFast cars: ${cars.length}`);
  ...
  const updatedCsvCount = await upsertRowsIntoCsv(rows);
  const updatedDbCount = await upsertRowsIntoSupabase(rows);

  console.log(`Updated CSV rows: ${updatedCsvCount}`);
  console.log(`Upserted Supabase rows: ${updatedDbCount}`);
}

async function fetchVinFastListings() {
  ...
  for (let page = 2; page <= pageCount; page += 1) {
    const url = `https://vnexpress.net/v-car/load-item/page_type/car_company/object_id/33/active_id/0/sort/asc/page_number/${page}`;
...
function parseListingPage(html: string) {
  ...
  for (const block of blocks) {
    ...
    const name = `VinFast ${decodeHtml(title).trim()}`;
...
function findVersionUrl(html: string) {
  const href = html.match(/class=["'][^"']*btn-thongso[^"']*["'][^>]+href=["']([^"']+)["']/i)?.[1] ?? html.match(/href=["']([^"']*\/oto-xe-may\/v-car\/phien-ban-xe\/[^"']+)["']/i)?.[1];
...
function parseSpecs(html: string) {
  ...
  for (const match of itemMatches) {
    ...
    const value = parseSpecValue(rawValue);
...
function buildCsvRow(car: ListingCar, specs: Map<string, SpecValue>, prices: { price: number; basePrice: number; topPrice: number }, detailHtml: string, versionHtml: string): CsvRow {
  ...
  const segment = cleanText(detailHtml.match(/<a class=["']tag["'][^>]*>([\s\S]*?)<\/a>/)?.[1] ?? "Ôtô điện");
  const engine = textSpec(specs, "Kiểu động cơ") || "Điện";
  const tireWheel = textSpec(specs, "Lốp, la-zăng") || "Chưa cập nhật";
  const safety = buildSafetySummary(specs);
  ...
  const row = {
    ...
    front_suspension: textSpec(specs, "Hệ thống treo trước") || "Chưa cập nhật",
    rear_suspension: textSpec(specs, "Hệ thống treo sau") || "Chưa cập nhật",
    engine,
    engine_hp: String(powerSpec(specs)),
    torque: String(numberSpec(specs, ["Mô-men xoắn môtơ điện (Nm)", "Mô-men xoắn cực đại (Nm)", "Mô-men xoắn kết hợp (Xăng+Điện) (Nm/rpm)", "Mô-men xoắn máy xăng/dầu (Nm)/vòng tua (vòng/phút)"], 0)),
    transmission: textSpec(specs, "Hộp số") || "1 cấp",
    fuel_consumption: "0.1",
    battery_type: textSpec(specs, "Loại pin") || "Chưa cập nhật",
    battery_capacity: String(numberSpec(specs, ["Dung lượng pin (kWh)"], 0)),
    acceleration_0_100: String(numberSpec(specs, ["Thời gian tăng tốc từ 0-100 km/h (giây)"], 0)),
    ...
    electric_mirrors: String(boolSpec(specs, "Gương chiếu hậu chỉnh điện") || boolSpec(specs, "Gương chiếu hậu gập điện")),
    ...
    leather_seats: String(hasText(specs, "Chất liệu bọc ghế", "Da")),
  ...
  return applyKnownFallbacks(row);
}

function applyKnownFallbacks(row: CsvRow) {
  if (row.name === "VinFast Minio Green 2026") {
    return {
      ...row,
      seats: "4",
      length: "3090",
      ...
      height: "1663",
      wheelbase: "2065",
      ground_clearance: "166",
      front_suspension: "MacPherson",
      rear_suspension: "MacPherson",
      front_brake: "Đĩa",
...
async function saveImage(url: string, outputPath: string) {
  const existing = await readdir(IMAGE_DIR).catch((): string[] => []);
  if (existing.includes(path.basename(outputPath))) return;
...
function parseDimensions(value: string) {
  const values = value.match(/[\d.]+/g)?.map((item) => Number(item.replace(/\./g, ""))) ?? [];
  ...
  for (const label of labels) {
    ...
    if (typeof value === "string") {
      ...
      if (Number.isFinite(parsed)) {
        if ((label.includes("(mm)") || label.includes("Chiều dài cơ sở")) && parsed > 0 && parsed < 20) {
